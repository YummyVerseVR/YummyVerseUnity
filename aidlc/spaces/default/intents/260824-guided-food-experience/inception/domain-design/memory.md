...
- QR role の変更は name replacement だけではない。`FoodContext`/download trigger と MenuSelectionBridge の責務を分離する。S3 の案内・QR 待機は現行 TutorialSequence から削除済みで、`QrPlateDetected` condition asset は履歴資産として runtime sequence から参照しない。
